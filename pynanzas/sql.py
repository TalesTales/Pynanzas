...
def crear_tabla_prods(nombre_tabla: str = TABLA_PRODS,
...
def crear_tabla_movs(nombre_tabla: str = TABLA_MOVS,
                     nombre_columnas: Optional[dict[str, str]] = None,
    ...
    for k, v in nombre_columnas.items():
        orden_ddl += f'{k} {v}'
        orden_ddl += ',\n'
    orden_ddl = orden_ddl + f"""FOREIGN KEY ({producto_id}) REFERENCES
    {nombre_tabla_prods} ({producto_id})"""
    try:
        with sqlite3.connect(nombre_bd) as conn:
            cursor: sqlite3.Cursor = conn.cursor()
            if not _tabla_prods_existe(cursor, nombre_tabla_prods):
